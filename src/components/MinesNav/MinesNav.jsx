...
import { handleMinins, minesFetch } from '../../store/slice/MinesSlice';
// Components
import IconButton from '@material-ui/core/IconButton';

// Styles
import './MinesNav.scss';

export const MinesNav = () => {
    const dispatch = useDispatch();
    const { isActive, isLoading, load } = useSelector((state) => state.MinesSlice);
    const active = isActive ? (!load ? '#575757' : '#ccc') : '#575757';

    const handleFeatchMines = () => {
        if (isActive) {
            dispatch(handleMinins(!isActive));
        } else {
            !load && dispatch(minesFetch());
            dispatch(handleMinins(!isActive));
        }
    };

    return (
        <div className="minesNav">
            <IconButton
